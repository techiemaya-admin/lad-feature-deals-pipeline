...
-- ✅ LAD Compliant: All tables properly tenant-scoped with metadata
...
CREATE INDEX IF NOT EXISTS idx_lead_notes_tenant_lead ON lead_notes(tenant_id, lead_id) WHERE is_deleted = FALSE;
CREATE INDEX IF NOT EXISTS idx_lead_attachments_tenant_lead ON lead_attachments(tenant_id, lead_id) WHERE is_deleted = FALSE;
CREATE INDEX IF NOT EXISTS idx_lead_stages_tenant_order ON lead_stages(tenant_id, display_order) WHERE is_deleted = FALSE;
...
BEGIN
  ...
  INSERT INTO leads (tenant_id, name, email, company, stage, status, estimated_value, priority) VALUES 
