...
CREATE TABLE IF NOT EXISTS lad_dev.lead_attachments (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
...
CREATE TABLE lad_dev.lead_bookings (
  ...
  -- Core relationships
  lead_id UUID NOT NULL REFERENCES lad_dev.leads(id),
  assigned_user_id UUID REFERENCES lad_dev.users(id), -- counsellor / agent
  ...
  parent_booking_id UUID REFERENCES lad_dev.lead_bookings(id),
...
CREATE INDEX idx_lead_bookings_tenant ON lad_dev.lead_bookings(tenant_id);
CREATE INDEX idx_lead_bookings_lead ON lad_dev.lead_bookings(lead_id);
CREATE INDEX idx_lead_bookings_user ON lad_dev.lead_bookings(assigned_user_id);
CREATE INDEX idx_lead_bookings_schedule ON lad_dev.lead_bookings(scheduled_at);
CREATE INDEX idx_lead_bookings_status ON lad_dev.lead_bookings(status);
